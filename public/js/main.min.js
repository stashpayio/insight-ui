...
var defaultLanguage = localStorage.getItem("insight-language") || "en",
    defaultCurrency = localStorage.getItem("insight-currency") || "STASH";
angular.module("insight", ["ngAnimate", "ngResource", "ngRoute", "ngProgress", "ui.bootstrap", "ui.route", "monospaced.qrcode", "gettext", "angularMoment", "insight.system", "insight.socket", "insight.blocks", "insight.transactions", "insight.address", "insight.search", "insight.status", "insight.connection", "insight.currency", "insight.messages"]), angular.module("insight.system", []), angular.module("insight.socket", []), angular.module("insight.blocks", []), angular.module("insight.transactions", []), angular.module("insight.address", []), angular.module("insight.search", []), angular.module("insight.status", []), angular.module("insight.connection", []), angular.module("insight.currency", []), angular.module("insight.messages", []), angular.module("insight.address").controller("AddressController", function ($scope, $rootScope, $routeParams, $location, Global, Address, getSocket) {
    ...
    $scope.$watch("dt", function (newValue, oldValue) {
        ...
        $rootScope.titleDetail = $scope.detail, Blocks.get({
            ...
            $scope.loading = !1, $scope.blocks = res.blocks, $scope.pagination = res.pagination, $scope.pagination.olderBlocks = $scope.pagination.moreTs;
            for (var key in $scope.blocks) $scope.blocks[key].time < $scope.pagination.olderBlocks && ($scope.pagination.olderBlocks = $scope.blocks[key].time)
        })
    ...
    $rootScope.currency.getConvertion = function (value) {
        ...
        }) : "mSTASH" === currency ? $rootScope.currency.factor = 1e3 : "uSTASH" === currency ? $rootScope.currency.factor = 1e6 : $rootScope.currency.factor = 1
...
angular.module("insight.system").controller("IndexController", function ($scope, Global, getSocket, Blocks) {
    ...
    qrcode.callback = function (data) {
        _scanStop();
        var str = 0 === data.indexOf("stash:") ? data.substring(8) : data;
...
ScopedSocket.prototype.removeAllListeners = function (opts) {
    ...
    gettextCatalog.setStrings("de_DE", {
        ...
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/stashpayio/insight/issues" target="_blank">github issue tracker</a>.': '<strong>insight</strong> befindet sich aktuell noch in der Entwicklung. Bitte sende alle gefundenen Fehler (Bugs) und Feedback zur weiteren Verbesserung an unseren <a href="https://github.com/stashpayio/insight-ui/issues" target="_blank">Github Issue Tracker</a>.',
        About: "Über insight",
        Address: "Adresse",
        Age: "Alter",
        "Application Status": "Programmstatus",
        "Best Block": "Bester Block",
        "Stash node information": "Stash-Node Info",
        Block: "Block",
        "Block Reward": "Belohnung",
        Blocks: "Blöcke",
        "Bytes Serialized": "Serialisierte Bytes",
        "Can't connect to stashd to get live updates from the p2p network. (Tried connecting to stashd at {{host}}:{{port}} and failed.)": "Es ist nicht möglich mit stashd zu verbinden um live Aktualisierungen vom P2P Netzwerk zu erhalten. (Verbindungsversuch zu stashd an {{host}}:{{port}} ist fehlgeschlagen.)",
        ...
        "Last Block Hash (stashd)": "Letzter Hash (stashd)",
        ...
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/stashpayio/insight/issues" target="_blank">github issue tracker</a>.': '<strong>insight</strong> esta en desarrollo aún, por ello agradecemos que nos reporten errores o sugerencias para mejorar el software. <a href="https://github.com/stashpayio/insight-ui/issues" target="_blank">Github issue tracker</a>.',
        About: "Acerca de",
        Address: "Dirección",
        Age: "Edad",
        "Application Status": "Estado de la Aplicación",
        "Best Block": "Mejor Bloque",
        "Stash node information": "Información del nodo Stash",
        Block: "Bloque",
        "Block Reward": "Bloque Recompensa",
        Blocks: "Bloques",
        "Bytes Serialized": "Bytes Serializados",
        "Can't connect to stashd to get live updates from the p2p network. (Tried connecting to stashd at {{host}}:{{port}} and failed.)": "No se pudo conectar a stashd para obtener actualizaciones en vivo de la red p2p. (Se intentó conectar a stashd de {{host}}:{{port}} y falló.)",
        ...
        "Last Block Hash (stashd)": "Último Bloque Hash (stashd)",
        ...
        '<strong>insight</strong>  is an <a href="http://live.insight.is/" target="_blank">open-source Stash blockchain explorer</a> with complete REST and websocket APIs that can be used for writing web wallets and other apps  that need more advanced blockchain queries than provided by stashd RPC.  Check out the <a href="https://github.com/stashpayio/insight-ui" target="_blank">source code</a>.': '<strong>insight</strong>  est un <a href="http://live.insight.is/" target="_blank">explorateur open-source de blocs</a> avec un ensemble complet d\'API REST et websockets qui peuvent être utilisées pour écrire portefeuille et autres applications nécéssitant des requêtes plus complète que celles proposés par les commandes RPC de stashd. Consultez le <a href="http://github.com/stashpayio/insight" target="_blank">code source sur GitHub</a>.',
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/stashpayio/insight/issues" target="_blank">github issue tracker</a>.': '<strong>insight</strong> est toujours en développement, hésitez pas à remonter tout bugs et proposer des améliorations sur notre <a href="https://github.com/stashpayio/insight-ui/issues" target="_blank">Github issue tracker</a>.',
        About: "À propos",
        Address: "Adresse",
        Age: "Âge",
        "Application Status": "Status de l'application",
        "Best Block": "Meilleur Bloc",
        "Stash node information": "Information sur le noeud Stash",
        Block: "Bloc",
        "Block Reward": "Recompense de bloc",
        Blocks: "Blocs",
        "Bytes Serialized": "Octets sérialisés",
        "Can't connect to stashd to get live updates from the p2p network. (Tried connecting to stashd at {{host}}:{{port}} and failed.)": "Impossible de se connecter à stashd pour récupérer les dernières informations du réseau p2p. (Tentative de connexion à stashd ({{host}}:{{port}}) échouée.)",
        ...
        "Last Block Hash (stashd)": "Hash du dernier bloc  (stashd)",
        ...
        "Best Block": "最良ブロック",
        "Stash comes with a way of signing arbitrary messages.": "Stashには任意のメッセージを署名する昨日が備わっています。",
        "Stash node information": "Stashノード情報",
        Block: "ブロック",
        "Block Reward": "ブロック報酬",
        Blocks: "ブロック",
        "Broadcast Raw Transaction": "生のトランザクションを配信",
        "Bytes Serialized": "シリアライズ後の容量 (バイト)",
        "Can't connect to stashd to get live updates from the p2p network. (Tried connecting to stashd at {{host}}:{{port}} and failed.)": "P2Pネットワークからライブ情報を取得するためにstashdへ接続することができませんでした。({{host}}:{{port}} への接続を試みましたが、失敗しました。)",
        ...
        Input: "入力",
        "Last Block": "直前のブロック",
        "Last Block Hash (stashd)": "直前のブロックのハッシュ値 (stashd)",
        ...
        "This form can be used to broadcast a raw transaction in hex format over\n        the Stash network.": "このフォームでは、16進数フォーマットの生のトランザクションをStashネットワーク上に配信することができます。",
        "This form can be used to verify that a message comes from\n        a specific Stash address.": "このフォームでは、メッセージが特定のStashアドレスから来たかどうかを検証することができます。",
        "Time Offset": "時間オフセット",
        Timestamp: "タイムスタンプ",
        Today: "今日",
        "Total Amount": "Stash総量",
        "Total Received": "総入金額",
        "Total Sent": "総送金額",
