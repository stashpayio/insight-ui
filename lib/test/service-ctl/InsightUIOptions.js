...
class InsightUIOptions extends NodeJsServiceOptions {
  ...
  mergeWithDefaultOptions(...customOptions) {
    ...
    const defaultContainerOptions = {
      image: 'stashpayio/insight',
      network: {
        name: 'stash_test_network',
        driver: 'bridge',
      },
      ports: [
        `${port}:${port}`,
      ...
      cmd: [
        '/insight/bin/stashcore-node',
        'start',
      ],
      volumes: [...InsightUIOptions.defaultCustomOptions.container.volumes,
        `${configFile}:/insight/stashcore-node.json`,
      ],
    };
  ...
  getConfigPath() {
    ...
    return path.join(tmpPath, 'bitcore-node-stash.json');
  }
